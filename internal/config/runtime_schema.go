...
const (
	FieldOwnerMGSCTL      FieldOwner = "mgsctl"
...
func DefaultRuntimeSchema() RuntimeSchema {
	return RuntimeSchema{
		...
		Fields: []RuntimeField{
			field("RUNTIME_SCHEMA_VERSION", "schema", "运行时配置 Schema 版本。升级工具使用它判断需要补充的字段。", "Runtime configuration schema version. Upgrade tooling uses it to add newly introduced fields.", "1", "1", FieldOwnerMGSCTL, requiredAlways, validatePositiveInteger),
			field("DEPLOYMENT_MODE", "deployment", "应用运行方式，可选 docker 或 native。", "Application runtime mode: docker or native.", "docker", "", FieldOwnerMGSCTL, requiredAlways, validateDeploymentMode),
			field("DEPLOYMENT_PROFILE", "deployment", "服务规格，可选 full、core 或 custom。原生部署不支持 full。", "Service profile: full, core, or custom. Native deployments do not support full.", "core", "", FieldOwnerMGSCTL, requiredAlways, validateDeploymentProfile),
			field("DEPLOYMENT_TOPOLOGY", "deployment", "部署拓扑，可选 single 或 cluster；集群拓扑中的后端节点必须使用 S3 对象存储。", "Deployment topology: single or cluster. Backend nodes in a cluster topology must use S3 object storage.", "single", "", FieldOwnerMGSCTL, requiredAlways, validateDeploymentTopology),
			field("DEPLOYMENT_ROLE", "deployment", "当前节点角色，可选 single、control、api、worker 或 web。", "Current node role: single, control, api, worker, or web.", "single", "", FieldOwnerMGSCTL, requiredAlways, validateDeploymentRole),
			field("DEPLOYMENT_MODULES", "deployment", "逗号分隔的本机模块清单，由部署工具维护。", "Comma-separated module list for this host, managed by the deployment tool.", "api,worker,user-web,admin-web,docs-web,gateway", "", FieldOwnerMGSCTL, requiredAlways, validateNonEmpty),

			field("POSTGRES_MANAGED", "ownership", "PostgreSQL 是否由当前 Docker 完整部署管理。", "Whether PostgreSQL is managed by this Docker full deployment.", "false", "false", FieldOwnerMGSCTL, requiredAlways, validateBool),
			field("REDIS_MANAGED", "ownership", "Redis 是否由当前 Docker 完整部署管理。", "Whether Redis is managed by this Docker full deployment.", "false", "false", FieldOwnerMGSCTL, requiredAlways, validateBool),
			field("OBJECT_STORAGE_MANAGED", "ownership", "对象存储是否由当前 Docker 完整部署管理。", "Whether object storage is managed by this Docker full deployment.", "false", "false", FieldOwnerMGSCTL, requiredAlways, validateBool),
			...
			secretField("SETUP_TOKEN", "setup", "首次初始化访问凭证；初始化完成前可重复使用，完成后会被移除并永久失效。", "First-run setup access credential; reusable until setup completes, then removed and permanently invalidated.", FieldOwnerMGSCTL, requiredSetupAuthority),
			field("SETUP_TOKEN_VERSION", "setup", "初始化访问凭证的单调递增版本。重置凭证时版本加一，用于立即作废旧凭证和会话；初始化完成后保留最后版本。", "Monotonically increasing setup credential version. Reset increments it to invalidate old credentials and sessions immediately; the final version is retained after setup completes.", "1", "1", FieldOwnerMGSCTL, requiredSetupAuthorityRole, validateRequiredPositiveUint64),

			field("POSTGRES_DATABASE", "managed middleware", "Docker 完整模式创建的 PostgreSQL 数据库名，仅由部署工具维护。", "PostgreSQL database name created in Docker full mode and managed only by the deployment tool.", "app", "", FieldOwnerMGSCTL, requiredDockerFull, validateNonEmpty),
			field("POSTGRES_USER", "managed middleware", "Docker 完整模式创建的 PostgreSQL 应用用户。", "PostgreSQL application user created in Docker full mode.", "app", "", FieldOwnerMGSCTL, requiredDockerFull, validateNonEmpty),
			secretField("POSTGRES_PASSWORD", "managed middleware", "Docker 完整模式创建的 PostgreSQL 用户密码。", "PostgreSQL user password created in Docker full mode.", FieldOwnerMGSCTL, requiredDockerFull),
			secretField("REDIS_PASSWORD", "managed middleware", "Docker 完整模式创建的 Redis 访问密码。", "Redis password created in Docker full mode.", FieldOwnerMGSCTL, requiredDockerFull),
			field("MINIO_ROOT_USER", "managed middleware", "Docker 完整模式创建的 MinIO 管理用户，仅用于资源准备。", "MinIO administrative user created in Docker full mode for resource provisioning.", "minio-admin", "", FieldOwnerMGSCTL, requiredDockerFull, validateNonEmpty),
			secretField("MINIO_ROOT_PASSWORD", "managed middleware", "Docker 完整模式创建的 MinIO 管理密码，仅用于资源准备。", "MinIO administrative password created in Docker full mode for resource provisioning.", FieldOwnerMGSCTL, requiredDockerFull),
			...
			secretField("AUTH_ACCESS_TOKEN_SECRET", "application secrets", "签发用户和管理员访问令牌的密钥；初始化后不得替换。", "Secret used to sign user and administrator access tokens; do not replace it after initialization.", FieldOwnerMGSCTL, requiredAPINode),
			secretField("API_KEY_SIGNING_SECRET_ENCRYPTION_KEY", "application secrets", "加密数据库中 API Key 签名材料的密钥；丢失后已有密钥无法恢复。", "Key encrypting API-key signing material in the database; losing it makes existing keys unrecoverable.", FieldOwnerMGSCTL, requiredAPINode),
			secretField("CASHIER_PROVIDER_CONFIG_ENCRYPTION_KEY", "application secrets", "加密支付渠道敏感配置的密钥；初始化后不得替换。", "Key encrypting sensitive payment-provider configuration; do not replace it after initialization.", FieldOwnerMGSCTL, requiredAPINode),
			secretField("PIC_GALLERY_SECURE_CONFIG_ENCRYPTION_KEY", "application secrets", "加密数据库中上游账号、对象存储和 SMTP 等敏感配置的共享密钥。", "Shared key encrypting provider, object-storage, SMTP, and other sensitive database configuration.", FieldOwnerMGSCTL, requiredBackend),
			secretField("PROMPT_OPTIMIZATION_QUOTE_SIGNING_KEY", "application secrets", "签名提示词优化报价的密钥；所有 API 节点必须一致。", "Key signing prompt-optimization quotes; it must be identical on every API node.", FieldOwnerMGSCTL, requiredAPINode),
			...
			field("PIC_GALLERY_DOCS_PROBE_URL", "public endpoints", "API 节点可回连的开发文档探测地址；由部署工具按节点拓扑生成，不在管理后台配置。", "Documentation probe URL reachable from the API node; generated by deployment tooling for the node topology and not editable in Admin.", "http://gateway/developer-docs/", "", FieldOwnerMGSCTL, requiredNever, validateDocsProbeURL),
			...
			field("GATEWAY_PORT", "ports", "可移植 Gateway 对外监听端口；未部署 Gateway 时可留空。", "Public listening port for the portable Gateway; leave empty when Gateway is not deployed.", "80", "", FieldOwnerMGSCTL, requiredNever, validateOptionalPort),
			field("USER_WEB_PORT", "ports", "用户前端模块的内部监听端口。", "Internal listening port for the user frontend module.", "5173", "", FieldOwnerMGSCTL, requiredNever, validateOptionalPort),
			...
			field("VIDEO_ARTIFACT_TEST_CA_FILE", "worker", "仅供 local/test 隔离验收使用：视频产物 HTTPS 服务的测试 CA PEM 文件。", "Local/test isolated acceptance only: test CA PEM file for the video artifact HTTPS service.", "", "", FieldOwnerMGSCTL, requiredNever, validateOptionalNonEmpty),
			...
			field("RELEASE_VERSION", "release", "原生发布包版本或本次部署选定的发行版本。", "Native release bundle version or the release selected for this deployment.", "v1.0.0", "", FieldOwnerMGSCTL, requiredNative, validateNonEmpty),

			field("INSTALLATION_ID", "identity", "当前安装的稳定唯一标识，所有集群节点必须一致。", "Stable unique installation identifier shared by every cluster node.", "019d0000-0000-7000-8000-000000000000", "", FieldOwnerMGSCTL, requiredAlways, validateIdentifier),
			field("CLUSTER_NODE_ID", "identity", "当前集群节点的唯一标识；single 节点可留空。", "Unique identifier of the current cluster node; single-node deployments may leave it empty.", "019d0000-0000-7000-8000-000000000001", "", FieldOwnerMGSCTL, requiredClusterNode, validateIdentifier),
			...
			field("APPLICATION_VERSION", "identity", "当前 API、Worker 或 Web 构建版本，用于加入和健康检查兼容性判断。", "Current API, Worker, or Web build version used for enrollment and health compatibility checks.", "v1.0.0", "", FieldOwnerMGSCTL, requiredAlways, ValidateApplicationVersion),
		},
	}
}
...
func (schema RuntimeSchema) Validate() error {
	...
	for _, runtimeField := range schema.Fields {
		...
		case FieldOwnerMGSCTL, FieldOwnerSetup, FieldOwnerApplication:
		default:
...
func validateDocsProbeURL(value string) error {
	if value == "" {
		return nil
	}
	parsed, err := url.Parse(value)
	if err != nil || !parsed.IsAbs() || parsed.Opaque != "" || parsed.User != nil || parsed.RawQuery != "" || parsed.Fragment != "" || strings.Contains(value, "\\") ||
		(parsed.Scheme != "http" && parsed.Scheme != "https") || parsed.Host == "" {
		return errors.New("documentation probe URL must be an absolute HTTP(S) URL without credentials, query, fragment, or backslashes")
	}
	return nil
}
