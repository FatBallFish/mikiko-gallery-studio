...
func TestValidateConnectivityUsesImplicitTLSOnPort465(t *testing.T) {
	...
	if err := ValidateConnectivity(context.Background(), cfg); err != nil {
		t.Fatalf("ValidateConnectivity: %v", err)
	}
}

func TestSendVerificationCodeUsesImplicitTLSOnPort465(t *testing.T) {
	addr, stop := startImplicitTLSSMTPServer(t)
	defer stop()
	host, port := splitHostPort(t, addr)
	markImplicitTLSPortForTest(t, port)
	...
	if err := SendVerificationCode(context.Background(), cfg, "user@example.com", "smtp_test", "000000"); err != nil {
...
func TestPort465DefaultsToImplicitTLS(t *testing.T) {
	if !usesImplicitTLS(config.SMTPConfig{Port: 465}) {
		t.Fatal("expected SMTP port 465 to default to implicit TLS")
	}
}
...
func validPlainAuth(line, username, password string) bool {
	...
	return string(payload) == "\x00"+username+"\x00"+password
}
