...
func TestAdminConfigTabEndpoints(t *testing.T) {
	...
	if err := authSvc.SendEmailCode("admin@example.com", "login"); err != nil {
		t.Fatalf("SendEmailCode: %v", err)
	}
	_, session, err := authSvc.LoginWithEmailCode("admin@example.com", "123456")
	if err != nil {
		t.Fatalf("LoginWithEmailCode: %v", err)
	}

	api := handlers.NewAPI(cfg, authSvc, nil)
