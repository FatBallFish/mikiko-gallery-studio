...
func TestMultimediaSchemasExposeRequiredContracts(t *testing.T) {
	tests := []struct {
		...
		{name: "video_task_items", fields: VideoTaskItem{}.Fields(), required: []string{"id", "task_id", "ordinal", "status", "stage", "result_asset_id", "actual_points", "next_action_at", "lease_owner", "lease_expires_at", "version"}},
