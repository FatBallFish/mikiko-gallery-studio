...
func (APIKey) Fields() []ent.Field {
	return []ent.Field{
		...
		field.String("secret_ciphertext").MaxLen(512).Optional().Nillable(),
		...
		field.String("total_quota_used_points").SchemaType(map[string]string{dialect.Postgres: "numeric(20,5)"}).Default("0.00000"),
		field.String("daily_quota_used_points").SchemaType(map[string]string{dialect.Postgres: "numeric(20,5)"}).Default("0.00000"),
		field.String("quota_usage_day").MaxLen(10).Optional().Nillable(),
		field.Int("rpm_limit").Optional().Nillable(),
		field.Time("rpm_window_started_at").Optional().Nillable(),
		field.Int("rpm_window_count").Default(0),
