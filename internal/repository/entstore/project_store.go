...
const defaultProjectBackfillLimit = 100
...
func (s *ProjectStore) Delete(ctx context.Context, userID int64, projectID, targetProjectID string, expectedVersion int64) (domainproject.DeleteResult, error) {
	...
	if source == nil || source.UserID != userID || !isActiveProject(source) {
		return domainproject.DeleteResult{}, repoerr.ErrNotFound
	}
	if source.IsDefault {
	...
	now := time.Now().UTC()
	deleted, err := tx.Project.UpdateOneID(sourceID).SetStatus(domainproject.StatusDeleted).SetDeletedAt(now).AddVersion(1).Save(ctx)
	if err != nil {
		return domainproject.DeleteResult{}, fmt.Errorf("soft-delete project: %w", err)
	}
	metadata := map[string]any{"tasks": counts.Tasks, "assets": counts.Assets}
	if targetProjectID != "" {
		metadata["target_project_id"] = targetProjectID
	}
...
func (s *ProjectStore) BackfillBatch(ctx context.Context, req domainproject.BackfillRequest) (domainproject.BackfillResult, error) {
	limit := req.Limit
	if limit <= 0 || limit > defaultProjectBackfillLimit {
		limit = defaultProjectBackfillLimit
	}
	users, err := s.client.User.Query().
		Where(userent.IDGT(int(req.AfterUserID)), userent.DeletedAtIsNil()).
		Order(repoent.Asc(userent.FieldID)).Limit(limit + 1).All(ctx)
	if err != nil {
		return domainproject.BackfillResult{}, fmt.Errorf("list project backfill users: %w", err)
	}
	done := len(users) <= limit
	if len(users) > limit {
		users = users[:limit]
	}
	result := domainproject.BackfillResult{Done: done}
	for _, user := range users {
		userID := int64(user.ID)
		project, ensureErr := s.EnsureDefault(ctx, userID)
		if ensureErr != nil {
			return result, fmt.Errorf("ensure default project for user %d: %w", userID, ensureErr)
		}
		projectID, parseErr := uuid.Parse(project.ID)
		if parseErr != nil {
			return result, fmt.Errorf("parse default project for user %d: %w", userID, parseErr)
		}
		tx, txErr := s.client.Tx(ctx)
		if txErr != nil {
			return result, fmt.Errorf("start project backfill for user %d: %w", userID, txErr)
		}
		if _, txErr = tx.ImageTask.Update().Where(imagetask.UserIDEQ(userID), imagetask.ProjectIDIsNil()).SetProjectID(projectID).Save(ctx); txErr == nil {
			_, txErr = tx.ImageResult.Update().Where(imageresult.UserIDEQ(userID), imageresult.ProjectIDIsNil()).SetProjectID(projectID).Save(ctx)
		}
		if txErr == nil {
			txErr = tx.Commit()
		} else {
		...
		if txErr != nil {
			return result, fmt.Errorf("backfill project ownership for user %d: %w", userID, txErr)
		}
		result.UsersProcessed++
		result.NextUserID = userID
	}
	return result, nil
}
