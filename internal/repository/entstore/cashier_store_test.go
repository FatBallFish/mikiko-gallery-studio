...
func TestCashierStorePersistsProviderInstancesInDedicatedTable(t *testing.T) {
	...
	created, err := store.CreateProviderInstance(ctx, domaincashier.ProviderInstanceWriteRequest{
	...
	updated, err := store.UpdateProviderInstance(ctx, created.ID, domaincashier.ProviderInstanceWriteRequest{
		ProviderInstance: domaincashier.ProviderInstance{
			ProviderType: "alipay_direct",
	...
	if instances[0].Config["app_id"] != "app-2" || instances[0].Config["app_private_key"] != "secret" {
		t.Fatalf("expected non-secret update to preserve old secret config, got %#v", instances[0].Config)
	}

	rotated, err := store.UpdateProviderInstance(ctx, created.ID, domaincashier.ProviderInstanceWriteRequest{
		ProviderInstance: domaincashier.ProviderInstance{
			ProviderType: "alipay_direct",
			Name:         "Alipay Sandbox B",
			Enabled:      false,
			Config:       map[string]any{"app_id": "app-3"},
		},
		Secrets: map[string]any{"app_private_key": "rotated-secret"},
	})
	if err != nil {
		t.Fatalf("UpdateProviderInstance with rotated secret: %v", err)
	}
	if rotated.Config["app_id"] != "app-3" || rotated.Config["app_private_key"] != "rotated-secret" {
		t.Fatalf("expected rotated secret in runtime config, got %#v", rotated.Config)
	}
