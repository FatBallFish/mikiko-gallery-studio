...
func (s *CashierStore) CreateProviderInstance(ctx context.Context, req domaincashier.ProviderInstanceWriteRequest) (domaincashier.ProviderInstance, error) {
	if s == nil || s.client == nil {
		return domaincashier.ProviderInstance{}, errs.Internal("cashier store is not available")
	}
	instance, err := cashierservice.ProviderInstanceForWrite(req, nil)
	if err != nil {
		return domaincashier.ProviderInstance{}, errs.BadRequest(err.Error())
	}
	normalized, err := cashierservice.NormalizeProviderInstance(instance, 0, time.Now().UTC())
	if err != nil {
		return domaincashier.ProviderInstance{}, errs.BadRequest(err.Error())
	}
...
func (s *CashierStore) UpdateProviderInstance(ctx context.Context, instanceID int64, req domaincashier.ProviderInstanceWriteRequest) (domaincashier.ProviderInstance, error) {
	if s == nil || s.client == nil {
		return domaincashier.ProviderInstance{}, errs.Internal("cashier store is not available")
	}
	if instanceID <= 0 {
	...
	current, err := s.client.PaymentProviderInstance.Get(ctx, int(instanceID))
	if err != nil {
	...
	instance, err := cashierservice.ProviderInstanceForWrite(req, s.decryptConfig(current.ConfigEncrypted))
	if err != nil {
		return domaincashier.ProviderInstance{}, errs.BadRequest(err.Error())
	}
	normalized, err := cashierservice.NormalizeProviderInstance(instance, instanceID, time.Now().UTC())
	if err != nil {
		return domaincashier.ProviderInstance{}, errs.BadRequest(err.Error())
	}
	configEncrypted, err := s.encryptConfig(normalized.Config)
