...
func Run(ctx context.Context, args []string, dependencies CLIDependencies) int {
	...
	case CommandSelfUpdate:
		...
		if !options.Yes {
			if dependencies.Terminal == nil || !dependencies.Terminal.Interactive() {
				fmt.Fprintln(dependencies.Stderr, "mgsctl: self-update requires an interactive terminal or --yes")
				return 2
			}
			confirmed, confirmErr := dependencies.Terminal.Confirm(ctx, fmt.Sprintf("Update mgsctl from %s to %s?", options.CurrentVersion, options.Version))
			if confirmErr != nil {
		...
		if result.Deferred {
			fmt.Fprintf(dependencies.Stdout, "Verified mgsctl %s. Windows replacement was scheduled after this process exits. Staged file: %s\n", result.CurrentVersion, result.StagedPath)
			return 0
		}
		fmt.Fprintf(dependencies.Stdout, "Updated mgsctl from %s to %s: %s\n", result.PreviousVersion, result.CurrentVersion, result.Executable)
		return 0
	...
	if input.Interactive {
		if dependencies.Terminal == nil || !dependencies.Terminal.Interactive() {
			fmt.Fprintln(dependencies.Stderr, "mgsctl: interactive install requires a terminal; provide all required flags with --yes")
			return 2
		}
...
func writeRunError(stderr io.Writer, err error) int {
	if errors.Is(err, context.Canceled) {
		fmt.Fprintln(stderr, "mgsctl: interrupted")
		return 130
	}
	fmt.Fprintf(stderr, "mgsctl: %v\n", err)
	return 1
}
