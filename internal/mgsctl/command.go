...
func (command Command) String() string {
	...
	case CommandClusterJoin:
		if command.ClusterJoin == nil {
			return "mgsctl cluster join"
		}
		return fmt.Sprintf("mgsctl cluster join --server %s --token <redacted>", command.ClusterJoin.Server)
	case CommandClusterTokenCreate:
		if command.ClusterTokenCreate == nil {
			return "mgsctl cluster token create"
		}
		return fmt.Sprintf("mgsctl cluster token create --role %s --ttl %s", command.ClusterTokenCreate.Role, command.ClusterTokenCreate.TTL)
	default:
		return "mgsctl " + string(command.Kind)
	}
}
...
func ParseCommand(args []string) (Command, error) {
	if len(args) == 0 {
		return Command{}, fmt.Errorf("a mgsctl command is required")
	}
	...
	default:
		return Command{}, fmt.Errorf("unknown mgsctl command %q", args[0])
	}
}

func parseSelfUpdateCommand(args []string) (Command, error) {
	set := newFlagSet("self-update")
	version := set.String("version", "latest", "target mgsctl version or latest")
	releaseBaseURL := set.String("release-base-url", DefaultMGSCTLReleaseBaseURL, "mgsctl release repository URL")
	downloadURL := set.String("download-url", "", "complete mgsctl artifact URL")
	expectedSHA256 := set.String("sha256", "", "expected mgsctl artifact SHA-256")
	yes := set.Bool("yes", false, "confirm the mgsctl tool replacement")
...
func parseInstallCommand(args []string) (Command, error) {
	...
	if resolvedRole == config.DeploymentRoleAPI || resolvedRole == config.DeploymentRoleWorker || resolvedRole == config.DeploymentRoleWeb {
		return Command{}, fmt.Errorf("joined roles must use mgsctl cluster join")
	}
	input := &InstallInput{
