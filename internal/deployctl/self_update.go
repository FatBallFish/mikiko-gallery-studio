...
const (
	DefaultDeployctlReleaseBaseURL = "https://github.com/fatballfish/pic-gallery/releases"
	maxDeployctlBinarySize         = 128 << 20
	maxDeployctlChecksumSize       = 4 << 10
)

var errDeployctlReleaseUnavailable = errors.New("deployctl release artifact is unavailable")
...
func ProductionSelfUpdateDependencies() SelfUpdateDependencies {
	return SelfUpdateDependencies{
		...
		GOOS: runtime.GOOS, GOARCH: runtime.GOARCH, Replace: replaceDeployctlExecutable,
	}
}

func ResolveDeployctlArtifact(options SelfUpdateOptions, goos, goarch string) (string, string, error) {
	if err := validateSelfUpdateOptions(options); err != nil {
		return "", "", err
	}
	if !supportedDeployctlPlatform(goos, goarch) {
		return "", "", fmt.Errorf("deployctl self-update does not support %s/%s", goos, goarch)
	}
	artifact := "deployctl-" + goos + "-" + goarch
	if goos == "windows" {
		artifact += ".exe"
	}
...
func SelfUpdate(ctx context.Context, options SelfUpdateOptions, dependencies SelfUpdateDependencies) (SelfUpdateResult, error) {
	...
	_, downloadURL, err := ResolveDeployctlArtifact(options, dependencies.GOOS, dependencies.GOARCH)
	if err != nil {
		return SelfUpdateResult{}, err
	}
	executable, err := dependencies.ExecutablePath()
	if err != nil {
		return SelfUpdateResult{}, fmt.Errorf("locate current deployctl executable: %w", err)
	}
	executable, err = filepath.Abs(executable)
	if err != nil {
		return SelfUpdateResult{}, fmt.Errorf("resolve current deployctl executable path: %w", err)
	}
	info, err := os.Stat(executable)
	if err != nil {
		return SelfUpdateResult{}, fmt.Errorf("inspect current deployctl executable: %w", err)
	}
	if !info.Mode().IsRegular() {
		return SelfUpdateResult{}, fmt.Errorf("current deployctl executable is not a regular file: %s", executable)
	}

	expected := strings.ToLower(strings.TrimSpace(options.ExpectedSHA256))
	if expected == "" {
		checksum, _, downloadErr := downloadBytes(ctx, dependencies.HTTPClient, downloadURL+".sha256", maxDeployctlChecksumSize)
		if downloadErr != nil {
			return SelfUpdateResult{}, releaseUnavailableError(downloadErr)
		}
		expected, err = parseDeployctlChecksum(checksum)
		if err != nil {
			return SelfUpdateResult{}, fmt.Errorf("parse deployctl release checksum: %w", err)
		}
	}

	staged, err := os.CreateTemp(filepath.Dir(executable), "."+filepath.Base(executable)+".update-*")
	if err != nil {
		return SelfUpdateResult{}, fmt.Errorf("stage deployctl update beside current executable: %w", err)
	}
	...
	actual, finalURL, err := downloadFile(ctx, dependencies.HTTPClient, downloadURL, staged, maxDeployctlBinarySize)
	if err != nil {
		return SelfUpdateResult{}, releaseUnavailableError(err)
	}
	if actual != expected {
		return SelfUpdateResult{}, fmt.Errorf("deployctl checksum verification failed: expected %s, got %s", expected, actual)
	}
	if err := staged.Chmod(info.Mode().Perm()); err != nil {
		return SelfUpdateResult{}, fmt.Errorf("apply deployctl executable permissions: %w", err)
	}
	if err := staged.Sync(); err != nil {
		return SelfUpdateResult{}, fmt.Errorf("sync staged deployctl executable: %w", err)
	}
	if err := staged.Close(); err != nil {
		return SelfUpdateResult{}, fmt.Errorf("close staged deployctl executable: %w", err)
	}

	deferred, err := dependencies.Replace(executable, stagedPath)
	if err != nil {
		keepStaged = true
		return SelfUpdateResult{}, fmt.Errorf("replace deployctl executable; verified update retained at %s: %w", stagedPath, err)
	}
	keepStaged = deferred
...
func validateSelfUpdateOptions(options SelfUpdateOptions) error {
	...
	if err := validateDeployctlDownloadURL(options.ReleaseBaseURL, true); err != nil {
		return fmt.Errorf("self-update release base URL: %w", err)
	}
	if options.DownloadURL != "" {
		if err := validateDeployctlDownloadURL(options.DownloadURL, false); err != nil {
...
func validateDeployctlDownloadURL(value string, base bool) error {
...
func supportedDeployctlPlatform(goos, goarch string) bool {
	return (goos == "linux" || goos == "darwin" || goos == "windows") && (goarch == "amd64" || goarch == "arm64")
}

func parseDeployctlChecksum(content []byte) (string, error) {
