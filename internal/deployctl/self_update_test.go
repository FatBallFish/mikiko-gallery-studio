...
func TestDeployctlArtifactURLMatchesPublishedPlatformNames(t *testing.T) {
	tests := []struct {
		goos, goarch, version, name, path string
	}{
		{"linux", "amd64", "latest", "deployctl-linux-amd64", "/latest/download/deployctl-linux-amd64"},
		{"darwin", "arm64", "v1.2.3", "deployctl-darwin-arm64", "/download/v1.2.3/deployctl-darwin-arm64"},
		{"windows", "amd64", "v2.0.0", "deployctl-windows-amd64.exe", "/download/v2.0.0/deployctl-windows-amd64.exe"},
	}
	for _, testCase := range tests {
		name, downloadURL, err := ResolveDeployctlArtifact(SelfUpdateOptions{
		...
		if err != nil || name != testCase.name || downloadURL != "https://example.test/releases"+testCase.path {
			t.Errorf("ResolveDeployctlArtifact(%s/%s %s) = %q, %q, %v", testCase.goos, testCase.goarch, testCase.version, name, downloadURL, err)
		}
	}
	...
	for _, options := range []SelfUpdateOptions{
		...
		if _, _, err := ResolveDeployctlArtifact(options, "linux", "amd64"); err == nil {
			t.Errorf("ResolveDeployctlArtifact(%#v) unexpectedly succeeded", options)
		}
	}
}

func TestSelfUpdateDownloadsVerifiesAndStagesReplacement(t *testing.T) {
	newBinary := []byte("new deployctl binary")
	digest := fmt.Sprintf("%x", sha256.Sum256(newBinary))
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		switch request.URL.Path {
		case "/download/v1.2.3/deployctl-linux-amd64":
			_, _ = writer.Write(newBinary)
		case "/download/v1.2.3/deployctl-linux-amd64.sha256":
			fmt.Fprintf(writer, "%s  deployctl-linux-amd64\n", digest)
		default:
	...
	directory := t.TempDir()
	executable := filepath.Join(directory, "deployctl")
	if err := os.WriteFile(executable, []byte("old deployctl binary"), 0o755); err != nil {
		t.Fatal(err)
	}
	replaced := false
...
func TestWindowsSelfUpdateScriptQuotesPathsAndPreservesRecoveryLog(t *testing.T) {
	script := windowsSelfUpdateScript(`C:\Program Files\deployctl's\deployctl.exe`, `C:\Program Files\deployctl's\deployctl.exe.new`, 1234)
	for _, required := range []string{"Wait-Process", "1234", "Move-Item", "deployctl''s", ".update-error.log"} {
