...
func InstallSummary(plan InstallPlan, result InstallResult, revealSetupToken bool) string {
	...
	if revealSetupToken {
		...
		fmt.Fprintf(&summary, "Setup token stored securely. Run deployctl setup token show --runtime-dir %q on the deployment host to display it.\n", plan.RuntimeDir)
	}
	...
	fmt.Fprintf(&summary, "  %d. Check health with deployctl status --runtime-dir %q and deployctl doctor --runtime-dir %q.\n", step, plan.RuntimeDir, plan.RuntimeDir)
