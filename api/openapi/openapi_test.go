...
func TestOpenAPISpecDocumentsAdminCallRecordOperationsContract(t *testing.T) {
	...
	for _, field := range []string{"task_id", "user_id", "api_key_id", "source_channel", "task_type", "status", "provider", "abstract_model", "quality", "estimated_points", "actual_points", "provider_cost", "gross_margin", "fallback_count", "route_snapshot_version", "error_code", "error_message", "attempt_count", "started_at", "finished_at"} {
