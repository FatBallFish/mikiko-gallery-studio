...
升级到 v0.0.25 时，启动迁移会幂等停用并软删除旧的视频组合成本、价格策略和价格规则，同时停用现有视频路由。迁移不会删除真实账号、真实模型、模型能力、路由候选、历史任务、报价快照、积分预留或账本流水；正在执行的旧任务继续按原 `pricing_snapshot` 结算。重复启动不会重复修改已经迁移的数据。
