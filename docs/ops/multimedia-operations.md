...
升级到 v0.0.25 时，启动迁移会幂等停用并软删除旧的视频价格策略和价格规则，同时停用现有视频路由。旧 Provider 组合成本规则不再进入新配置、v2 报价或管理后台，但其数据库记录会保持原启用状态，仅供升级前已创建的 legacy 任务在重试或创建后续 attempt 时读取成本快照；在所有旧任务终态且超过审计保留期后，才能通过后续独立迁移清理。迁移不会删除真实账号、真实模型、模型能力、路由候选、历史任务、报价快照、积分预留或账本流水；正在执行的旧任务继续按原 `pricing_snapshot` 结算。重复启动不会重复修改已经迁移的数据。

升级完成后，管理员必须按以下顺序恢复视频入口：

1. 在“接入账号 -> 视频”逐个检查真实模型能力，为 Seedance 配置各分辨率的 CNY/百万 Token 销售单价，为 MiniMax H3 配置输出/输入视频 CNY/秒、免费图片数和超额图片 CNY/张。
2. 在“路由模型 -> 视频”检查候选与参数映射，特别确认公开 `720p` 到 MiniMax 原生 `768p` 的映射；不支持的组合不得对用户开放。
3. 在“价格策略 -> 视频报价总览”设置最低任务积分和 `1/5/10` 积分向上取整步长。积分换算只读取系统设置中的 `billing_pricing.cny_per_point`。
4. 对每个路由运行报价试算，确认每个候选的支持状态、CNY 报价、最高价来源和最终积分；缺少费率或能力的候选必须先修复。
5. 试算无阻断项后重新启用视频路由，再执行一次正式报价与测试任务。Provider 实际 Token/秒数仅用于审计，不改变已锁定积分。

如果需要回滚代码，不要恢复或猜测旧组合价格。保持视频路由停用，保留新增费率卡和全部历史任务/账务数据，待兼容版本重新部署后再按报价试算结果启用。旧任务的 legacy 结算分支必须保留到所有旧任务终态且超过审计保留期。
