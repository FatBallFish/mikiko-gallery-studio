...
- 验证结果：
  ...
  - 调用记录前置失败验收继续补强：新增 `TestCreateTaskPersistsFailedCallRecordWhenRoutePriceMissing`，证明路由模型存在但价格缺失时会持久化 `failed` 任务记录、错误码为 `ROUTE_MODEL_PRICE_MISSING` 且点数快照为 `0.00000`；后台调用记录展示模型同步将余额不足提示从旧 `INSUFFICIENT_BALANCE` 修正为真实后端枚举 `BILLING_INSUFFICIENT_POINTS`，`callRecordRows.contract.ts` 锁定价格缺失与余额不足两类前置失败的运营排障文案。
  - 调用记录“无模型/无候选”验收补齐到服务层：新增 `TestCreateTaskPersistsFailedCallRecordWhenRouteHasNoCandidate`，构造 route model 与价格存在但候选账号禁用的场景，要求创建任务返回 `MODEL_ROUTE_NO_CANDIDATE` 并持久化 `failed` 调用记录，保留 `route_model_code/resolved_quality_bucket` 与零点数快照；`callRecordRows.contract.ts` 同步锁定 `MODEL_ROUTE_NO_CANDIDATE` 的后台排障文案指向候选账号配置。
  ...
  - Phase 2 收银台 provider 配置模型继续从 handler 下沉：`internal/service/cashier/provider.go` 已承载 provider instance 归一化、默认支持方式、金额限额格式化、密钥字段识别、配置脱敏和 `credentials_status` 指纹生成；后台 provider instance 创建/更新和响应 payload 继续保持原 HTTP 契约，但不再在 handler 内维护第二套密钥遮蔽规则。
  - Phase 2 收银台/套餐 schema 继续向方案契约对齐：`subscription_plans` 已新增正式 `plan_type` 与 `purchase_enabled` Ent 字段，Ent 生成代码与 schema 测试同步更新；`BillingStore` 创建、更新、默认 seed 与响应映射改为写入正式字段，同时保留 metadata 兼容旧数据，支撑“积分包开放购买、订阅套餐保留但不开放”的后端持久化契约不再只依赖 JSON metadata。
  - 后台主动查单结果在迁入 `internal/service/cashier.QueryOrderStatusResult` 后已补齐 JSON tag，保持 `/api/ops/admin/v1/cashier/orders/{order_id}/sync` 的 `query_status/risk_category/action_hint/trade_no/amount_cny` HTTP 响应契约不变；handler 测试同步改为验证 `cashierservice.NormalizeQueryStatus`，避免旧 handler-local 归一化函数残留。
  - `go test ./internal/http/handlers ./internal/http/router ./internal/service/cashier ./internal/repository/entstore -count=1`、`go test ./internal/repository/ent/schema -count=1`、`go test ./api/openapi -count=1` 在补齐套餐正式字段和 sync 响应 JSON 契约后通过。
  - Phase 2 收银台 provider instance 持久化继续向方案契约对齐：新增 `payment_provider_instances` Ent schema、生成代码与 `internal/repository/entstore.CashierStore`，运行态 API 通过 `SetCashierProviderInstanceStore(entstore.NewCashierStore(client))` 优先读写专表；`ConfigFacade` 保留旧 `payments.provider_instances` JSON 配置作为未注入 store 时的兼容路径，后台/用户收银台 HTTP 契约不变。
  - `payment_provider_instances.config_encrypted` 已补齐 AES-GCM 静态加密：运行态通过 `entstore.NewCashierStoreWithConfigEncryptionKey(client, cfg.Cashier.ProviderConfigEncryptionKey)` 注入收银台专用密钥，创建/更新 provider instance 时落库为 `v1:` ciphertext 包装；store 读取时解密还原给支付 adapter 使用，并兼容旧明文 JSON。`internal/repository/entstore/cashier_store_test.go` 已锁定原始 Ent 行不得包含 `app_private_key`/密钥明文，同时 `ProviderInstances` 仍可还原运行态配置。
  - 钱包桶索引契约已补齐：`wallet_grants` 新增 `user_id,status,grant_type,expires_at` 复合索引，支撑按用户/状态/桶类型/过期时间扫描；`wallet_reservation_allocations` 新增 `wallet_grant_id,task_id,reservation_cycle` 唯一索引，防止同一任务同一预扣轮次重复绑定同一 grant。`internal/repository/ent/schema/schema_test.go` 已锁定两项 schema 契约，Ent 生成代码已同步。
  - 体验额度过期提醒阈值已配置化：注册送体验额度时将 `expiry_reminder_days` 写入 trial grant metadata，Ent BillingStore 汇总余额桶时读取对应 grant 的提醒阈值，不再硬编码 48 小时；用户端继续消费 `balance.buckets[].expire_warning` 展示“即将过期”。`TestBillingStoreSignupTrialExpiryWarningUsesGrantReminderDays` 与 `TestEmailCodeLoginUsesAdminSignupTrialExpiryReminderDays` 已锁定后台配置到余额桶提醒的链路。
  - 退款 adapter 下沉补齐 handler hookup：`refundCashierOrderWithProvider` 通过 `internal/service/cashier.RefundAdapterRegistry` 分发到支付宝、微信、易支付、JeePay builder，builder 复用既有真实渠道退款实现，保持渠道请求和本地预检/冻结/落账语义不变。
  - `go test ./internal/repository/ent/schema -run 'TestPaymentProviderInstanceSchemaCarriesCashierContract|TestSubscriptionPlanSchemaCarriesPurchaseTypeContract' -count=1`、`go test ./internal/repository/entstore -run TestCashierStorePersistsProviderInstancesInDedicatedTable -count=1`、`go test ./internal/http/router -run 'TestAdminCashierProviderInstanceCreateAndUpdate|TestCashierRoundRobinSchedulesAcrossProviderInstances|TestCashierOrderSchedulesConfiguredProviderInstance|TestAdminCashierReadEndpoints' -count=1`、`go test ./internal/http/handlers ./internal/service/cashier ./internal/repository/entstore -count=1`、`go test ./api/openapi -count=1` 在补齐 provider instance 专表和退款 adapter hookup 后通过。
  - 收银台待支付订单关闭链路已补齐到后台页面：`CashierPage.tsx` 对 `pending` 订单提供“关闭”操作，调用 `/api/ops/admin/v1/cashier/orders/{order_id}/close` 后回写订单列表和详情；后端测试 `TestAdminCashierOrderCloseCancelsPendingOrderAndBlocksCompletion` 锁定关闭后状态为 `canceled`、写入 `closed_at`、不能再人工补单且不会给用户充值余额。
  - 用户端收银台创建订单已显式复用一次提交的 `Idempotency-Key`，重复点击/网络重放会命中同一订单；创建成功后才生成下一次提交的新 key，避免前端默认每次调用都换 key 导致多笔 pending 订单。
  - `go test ./internal/http/router -run 'TestAdminCashierOrder(CloseCancelsPendingOrderAndBlocksCompletion|CompleteManuallyCreditsRechargeBalance)|TestCashierCreateOrderReusesIdempotencyKey' -count=1`、`npm --prefix web/user run typecheck`、`npm --prefix web/admin run typecheck`、`npm --prefix web/user run build`、`npm --prefix web/admin run build`、`go test ./api/openapi -count=1` 在补齐订单关闭入口和用户下单幂等后通过。
- 浏览器手动验收补证：
...
- 最新收口状态（2026-06-07）：
  - 公开广场列表接口已补齐 `query` 查询参数：`GET /api/open/image/v1/gallery/images?query=...` 会在公开图片 ID、列表摘要、模型、任务类型和作者范围内检索，保持列表不返回、不索引完整 prompt；前端 `PublicGalleryPage.tsx` 搜索框已改为服务端查询，避免只在当前 40 条结果内本地筛选。`TestGalleryPublishReviewAndPublicListFlow` 与 `TestOpenAPISpecDocumentsPublicGalleryPromptBoundaryContract` 已覆盖运行态和 OpenAPI 契约。
  - 收银台支付渠道实例密钥已从复用 API Key 加密密钥改为独立 `cashier.provider_config_encryption_key`，支持 `CASHIER_PROVIDER_CONFIG_ENCRYPTION_KEY` / `PIC_GALLERY_CASHIER_PROVIDER_CONFIG_ENCRYPTION_KEY` 环境变量；`configs/config*.yaml`、`deployments/docker-compose/.env.example`、`docker-compose.e2e.yml`、`scripts/test/api_contract_smoke.sh` 和 `docs/deploy/backend-runbook.md` 已同步配置入口。
  - 统一验证已通过：`./scripts/workflow/verify.sh` 覆盖 Go tests/vet、前端 contracts、用户端与后台 typecheck/build；`./scripts/workflow/api-smoke.sh` 真实 API smoke 已通过。
  - Docker E2E 脚本与 compose 配置已做可运行前检查：`bash -n scripts/test/api_contract_smoke.sh scripts/e2e/run-docker-e2e.sh`、`node --check scripts/e2e/docker-e2e.mjs`、`docker compose -f deployments/docker-compose/docker-compose.e2e.yml config` 均通过；实际 Docker E2E 仍受本机 Docker daemon 未运行阻塞。
  - Review gate 当前为流程性 BLOCK：`.coding-context.json` 是 heavyweight 任务且 `approval.status=pending`，需审批通过后才能生成 push/PR ready 的 PASS marker。
