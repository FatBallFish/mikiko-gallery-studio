...
for required in \
  'Gallery references always create non-owning aliases to the original object' \
  'There is no runtime rollout switch' \
  'Before rolling back, verify the target release remains alias-aware and protects shared objects during cleanup'; do
  require_text "$ROOT/docs/runbooks/backend-deployment.md" "$required"
done
...
forbid_text "$ROOT/docs/runbooks/backend-deployment.md" 'runtime_rollout/no_copy_reference_aliases'
