...
case "$target_os" in
  ...
  *) echo "unsupported deployctl release OS: $target_os" >&2; exit 2 ;;
esac
case "$target_arch" in
  amd64|arm64) ;;
  *) echo "unsupported deployctl release architecture: $target_arch" >&2; exit 2 ;;
esac

extension=""
[[ "$target_os" == windows ]] && extension=".exe"
artifact="deployctl-${target_os}-${target_arch}${extension}"
mkdir -p "$target_root"

GOOS="$target_os" GOARCH="$target_arch" CGO_ENABLED=0 \
  make -C "$ROOT" deployctl \
    "DEPLOYCTL_OUTPUT=$target_root/$artifact" \
    "DEPLOYCTL_VERSION=$version" \
    "DEPLOYCTL_COMMIT=$commit" \
    "DEPLOYCTL_BUILD_TIME=$build_time" \
...
if command -v sha256sum >/dev/null 2>&1; then
  ...
  echo "sha256 tool is required to package deployctl" >&2
  exit 1
fi
