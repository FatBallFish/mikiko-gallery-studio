...
export function createCanvasStore(document: CanvasDocument, revision: number, options: { recoveredDraft?: boolean } = {}) {
  return createStore<CanvasStoreState>((set) => ({
    ...
    select: (selectedIDs) => set({ selectedIDs: Array.from(new Set(selectedIDs)) }),
    setViewport: (viewport) => set((state) => ({ command: { ...state.command, present: { ...state.command.present, viewport }, dirty: true } })),
    addNode: (node) => set((state) => ({ command: addCanvasNode(state.command, node), selectedIDs: [node.id] })),
    ...
    pasteClipboard: () => set((state) => {
      ...
      return { command, selectedIDs: command.present.nodes.slice(-state.clipboard.nodes.length).map((node) => node.id), pasteCount: state.pasteCount + 1 }
    }),
    deleteSelected: () => set((state) => ({ command: removeCanvasNodes(state.command, state.selectedIDs), selectedIDs: [] })),
    ...
    replaceRemote: (nextDocument, nextRevision) => set({ command: createCanvasState(nextDocument, nextRevision), selectedIDs: [], pasteCount: 0 }),
