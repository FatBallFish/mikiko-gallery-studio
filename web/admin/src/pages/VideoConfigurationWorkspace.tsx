...
type Draft = {
  ...
  taskType: string; resolution: string; ratio: string; audioMode: string; duration: string; salesPoints: string; pricingMode: 'exact' | 'metered'
...
const initialDraft: Draft = {
  ...
  taskType: 'text_to_video', resolution: '720p', ratio: '16:9', audioMode: 'silent', duration: '5', salesPoints: '0', pricingMode: 'exact', routeVersion: 'video-route-v1', maxOutput: '4',
...
export function VideoConfigurationWorkspace({ context }: { context: Context }) {
  ...
  const currentPrice = useMemo(() => snapshot?.price_rules
    .filter((item) => String(item.pricing_strategy_id) === draft.strategyID && item.task_type === draft.taskType && item.resolution === draft.resolution && item.audio_mode === draft.audioMode)
    .sort((left, right) => right.rule_version - left.rule_version)[0], [snapshot, draft.strategyID, draft.taskType, draft.resolution, draft.audioMode])
  ...
  useEffect(() => {
    ...
    setDraft((current) => ({ ...current, pricingMode: currentPrice.pricing_mode, salesPoints: currentPrice.sales_points }))
  }, [currentPrice])
  ...
  async function savePrice() {
    const input = { route_model_id: Number(draft.routeID), pricing_strategy_id: Number(draft.strategyID), expected_version: currentPrice?.rule_version ?? 0, task_type: draft.taskType, resolution: draft.resolution, audio_mode: draft.audioMode, duration_seconds: Number(draft.duration), effective_at: new Date().toISOString(), minimum_task_points: draft.salesPoints, pricing_mode: draft.pricingMode, enabled: draft.enabled }
    await run(() => currentPrice ? adminApi.updateVideoPriceRule(currentPrice.id, input) : adminApi.createVideoPriceRule(input), '销售价格版本已保存')
  }
  ...
  return <section className="grid gap-4 border-b border-[var(--border)] pb-5" data-video-configuration-workspace={context}>
    ...
    {context === 'pricing' ? <div className="grid gap-3 lg:grid-cols-3">
      ...
      <Field label="计费方式"><select value={draft.pricingMode} onChange={(event) => update('pricingMode', event.target.value as Draft['pricingMode'])}><option value="exact">按请求参数精确结算</option><option value="metered">按实际输出时长结算</option></select></Field>
