...
export function VideoConfigurationImpact({ context }: { context: 'models' | 'pricing' | 'routing' }) {
  ...
  const links = context === 'models' ? [['pricing', '查看视频价格'], ['routing', '查看视频路由']] : context === 'pricing' ? [['access-accounts', '查看真实模型成本'], ['routing', '查看受影响路由']] : [['access-accounts', '查看真实模型能力'], ['pricing', '查看绑定价格']]
  return <section className="grid gap-3 border-y border-[var(--border)] py-4" data-video-configuration-impact>
    <div className="flex flex-wrap items-center justify-between gap-3"><div><strong className="text-sm text-[var(--text)]">视频配置影响摘要</strong><p className="m-0 mt-1 text-xs text-[var(--soft)]">能力 {snapshot.capabilities.length} 版 · 成本 {snapshot.cost_rules.length} 版 · 价格策略 {snapshot.pricing_strategies.length} 版 · 路由 {snapshot.routes.length} 版；四类配置独立版本化。</p></div><div className="flex flex-wrap gap-2">{links.map(([href, label]) => <a key={href} className={cn(adminButton.base, adminButton.ghost, adminButton.small)} href={`#/${href}`}>{label}</a>)}</div></div>
    {blocking.length ? <InlineFeedback tone="danger" message={`${blocking.length} 项阻断视频上线：${blocking.slice(0, 3).map((item) => item.summary).join('；')}`} /> : <InlineFeedback tone="success" message="当前未发现候选缺失、缺价或低于安全线的启用视频配置。" />}
  </section>
}
