...
export const pricingFieldHints = {
  ...
  basePoints: '单张图片在该路由模型、任务类型和质量档位下的基础积分，支持 5 位小数。',
...
export const pricingQualityOptions = [
  { value: '1K', label: pricingQualityLabel('1K') },
  { value: '2K', label: pricingQualityLabel('2K') },
  { value: '4K', label: pricingQualityLabel('4K') },
] as const
...
export function pricingQualityLabel(quality?: string | null) {
  const normalized = normalize(quality)
  return qualityLabels[normalized] ?? ((quality ?? '').trim() || '未知质量')
}
