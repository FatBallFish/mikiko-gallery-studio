      ...
      <section className="pg-admin-card ops-surface full-main">
        <section className="main-lane table-lane">
          ...
          {!accounts.length ? <EmptyBlock title="暂无模型接入账号" detail="创建账号后再添加真实上游模型。" /> : (
            <div className="admin-data-grid account-grid">
              <div className="table-head"><span>账号</span><span>接入方式</span><span>Base URL</span><span>调度</span><span>状态</span><span>操作</span></div>
              {accounts.map((row) => (
                <div key={String(row.id)} className="table-row">
                  <button type="button" className="text-button" onClick={() => setSelectedAccountId(String(row.id))}><strong>{row.name}</strong><p>{credentialsStatusLabel(row.credentials_status?.has_api_key)}</p></button>
        ...
        <aside className="signal-rail">
          ...
          {selectedModels.length ? selectedModels.map((model) => (
            <section key={String(model.id)} className="signal-section">
              ...
              <div className="row-actions buttons"><Badge tone={modelEnabledTone(model.enabled)}>{modelEnabledLabel(model.enabled)}</Badge><button className="ghost small" type="button" onClick={() => selectedAccount && setModelDialog(editModelDraft(selectedAccount, model))}>编辑</button></div>
            </section>
      ...
      {modelDialog ? (
        <Modal title={modelDialog.row ? '编辑真实模型' : '新增真实模型'} detail={modelDialog.account.name} onClose={() => setModelDialog(null)} footer={<><button className="ghost" type="button" disabled={saving} onClick={() => setModelDialog(null)}>取消</button><button className="btn primary" type="button" disabled={saving || !modelDialog.modelCode} onClick={() => void saveModel()}>{saving ? '保存中...' : '保存'}</button></>}>
          <div className="form-grid">
            ...
            <Field label="任务类型"><div className="check-grid-scroll">{adminTaskTypeOptions.map((option) => <label key={option.value} className="check-option"><input type="checkbox" checked={modelDialog.taskTypes.includes(option.value)} onChange={(event) => setModelDialog({ ...modelDialog, taskTypes: event.target.checked ? [...modelDialog.taskTypes, option.value] : modelDialog.taskTypes.filter((item) => item !== option.value) })} /><span>{option.label}</span></label>)}</div></Field>
